...
# Ensure we don't need a GUI backend for matplotlib
import matplotlib
import random
...
# Function to save 10 random images from the training dataset in one file
def save_sample_images(dataset, file_path, num_images=10):
    ...
    for i, (image, label) in enumerate(images_labels):
        ...
        plt.axis('off')
        plt.title(''.join([label_to_char(l.item()) for l in label]))

    plt.savefig(file_path, bbox_inches='tight')
    plt.close()

# Save 10 random training images
save_sample_images(train_dataset, os.path.join(image_dir, 'sample_training_images.png'))
...
plt.savefig(os.path.join(image_dir, 'training_validation_curves.png'), bbox_inches='tight')
plt.close()

print(f"Training completed. Plots saved in {image_dir}/training_validation_curves.png")
print(f"Sample training image saved in {image_dir}/sample_training_image.png")
